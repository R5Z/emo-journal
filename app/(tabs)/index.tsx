...
import { 
  View, Text, StyleSheet, ScrollView, TouchableOpacity, PanResponder, StyleSheet as RNStyleSheet, Platform
} from 'react-native';
  ...
  const panResponder = useMemo(() => PanResponder.create({
    ...
    onPanResponderRelease: (_, gestureState) => {
      ...
      if (!isExpended) {
        ...
        if (dy > 70) setSelectedDate(dayjs(selectedDate).subtract(1, 'month').startOf('month').format('YYYY-MM-DD'));
        else if (dy < -70) setSelectedDate(dayjs(selectedDate).add(1, 'month').startOf('month').format('YYYY-MM-DD'));
      ...
      <View {...panResponder.panHandlers} style={styles.calendarHeader}>
        ...
        <View style={styles.monthHeaderRow}>
          ...
          <TouchableOpacity onPress={handleGoToToday} style={styles.todayButton}>
            <View style={styles.todayIcon}><Text style={styles.todayIconText}>{todayNum}</Text></View>
          </TouchableOpacity>
        </View>
        {!isExpended ? (
            ...
            <View style={styles.monthGrid}>
              {calendarGrid.map((date) => {
                ...
                return (
                  ...
                  >
                    <View style={[styles.gridInner, isSelected && styles.gridSelected, !isCurrentMonth && { opacity: 0.3 }, isFuture && { opacity: 0.1 }]}>
                      {emotion && <LinearGradient colors={emotion.colors} style={RNStyleSheet.absoluteFill} borderRadius={8} />}
...
const styles = StyleSheet.create({
  ...
  todayIcon: { width: 30, height: 30, backgroundColor: '#007AFF', borderRadius: 8, justifyContent: 'center', alignItems: 'center', elevation: 2 },
